...
NN(1)                 = 17;                               % number of nodes in X direction
NN(2)                 = 17;                               % number of nodes in Y direction
...
for sp = 1:spCount  
    velocity_data{sp,1} = [0.1;0];
    velocity_data{sp,2} = [0.1;0];
    velocity_data{sp,3} = [0.1;0];
    velocity_data{sp,4} = [0.1;0];
    velocity_data{sp,5} = [0.1;0];
...
writerObj2           = VideoWriter('Rebound.avi');
...
%   
%     figure
%     plot(time,total_elems,time,Gauss_elems,time,MPM_elems)
% %     axis([0,ftime,0,100])
%     ylabel('number of elements')
%     xlabel('time')
%     legend('total','Gauss','MPM')
%     
