function[N_MLS,singular_node]=LinearNode_IMLS(spCount,nodeCount,npoints1,CONNECT_TEMP,x_sp,LOC,le)
...
for n = 1:nodeCount
     ...
     % Compute the weight function wn
     for i = 1:Np
         pid = npoints1{n}(i);
...
%          Nn_local(pid,n) = Cubic_Bspline(x_sp(pid,1:2),LOC(n,:),le(1),le(2));
     ...
     p = [won;cv];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Compute the monomial basis function p
q=zeros(3,Np);
gama=zeros(1,3); % (qk,qk)          k = 1:i-1
bita=zeros(1,3); % (pi,qk)
Rfa=zeros(1,3);  % (pi,qk)/(qk,qk)

% First polynomial q1
q(1,1:Np)=1;

% Second polynomial q2
p1xi=p(1,1:Np);
q1xi=q(1,1:Np);
p2xi=p(2,1:Np);
gama1=0;
